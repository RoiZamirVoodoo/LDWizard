...
def _detect_steep_drops(df):
    ...
    relative to a rolling average of nearby levels.
    """
    ...
    steep = []
    # Use a rolling window to compute local average
    window = 20
    for i in range(n):
        # Local window (excluding current level)
        ...
        if current < 0.005 or local_avg < 0.001:
            continue

        ratio = current / local_avg if local_avg > 0 else 0
...
def _generate_funnel_insights(df, zones, steep_drops, cadence, pacing_score):
    ...
    if critical_drops:
        level_list = ", ".join([f"L{d['level']}" for d in critical_drops[:5]])
        suffix = f" and {len(critical_drops) - 5} more" if len(critical_drops) > 5 else ""
        insights.append(f"{len(critical_drops)} critical drop-off spike(s) detected at: {level_list}{suffix}.")
