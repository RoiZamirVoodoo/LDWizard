"""
LD Wizard — Phase 3: Drop-off Analysis
Identifies drop-off spikes, classifies drop-off zones, correlates drop-offs
with difficulty and churn, and provides actionable level-specific insights.

Phase-aware: uses deviation-from-expected baseline so early levels (which
naturally churn higher) are not over-flagged, while late-game drops get
amplified attention.
"""

import pandas as pd
import numpy as np
from engine.parser import DIFFICULTY_ORDER, FUNNEL_PHASES


# ---------------------------------------------------------------------------
# Configuration
# ---------------------------------------------------------------------------

# Rolling window for smoothing and spike detection
ROLLING_WINDOW = 5

# Spike thresholds — now based on phase-adjusted deviation
# A spike is flagged when the adjusted deviation exceeds these multiples
# of the adjusted standard deviation within the phase.
SPIKE_WARNING_SIGMA = 1.8
SPIKE_CRITICAL_SIGMA = 3.0

# Zone classification thresholds (cumulative user loss within a zone)
ZONE_LOSS_WARNING = 0.05   # 5% funnel loss in a single zone
ZONE_LOSS_CRITICAL = 0.10  # 10% funnel loss


def compute_dropoff_analysis(df):
    """
    Full drop-off analysis with phase-aware deviation baseline.

    Args:
        df: Enriched DataFrame from parser

    Returns:
        dict with keys:
            - dropoff_by_level: list per level with drop-off metrics + deviation data
            - spikes: list of detected drop-off spikes (phase-adjusted)
            - zones: list of drop-off concentration zones
            - phase_summary: per-phase aggregated drop-off stats
            - churn_correlation: correlation between drop-off and churn metrics
            - bracket_dropoff: average drop-off by bracket
            - insights: list of string insights
    """
    required = ["level", "users", "target_bracket"]
    missing = [c for c in required if c not in df.columns]
    if missing:
        return {
            "dropoff_by_level": [], "spikes": [], "zones": [],
            "phase_summary": [], "churn_correlation": {}, "bracket_dropoff": [],
            "insights": [f"Missing columns: {', '.join(missing)}"],
        }

    df = df.copy().sort_values("level").reset_index(drop=True)

    # Ensure dropoff_rate exists
    if "dropoff_rate" not in df.columns:
        df["dropoff_rate"] = (df["users"] - df["users"].shift(-1)) / df["users"]
        df.loc[df.index[-1], "dropoff_rate"] = 0

    # Absolute user loss per level
    df["user_loss"] = df["users"] - df["users"].shift(-1)
    df.loc[df.index[-1], "user_loss"] = 0

    # Rolling average for display (still useful for the chart)
    df["dropoff_rolling_avg"] = df["dropoff_rate"].rolling(
        window=ROLLING_WINDOW, center=True, min_periods=3
    ).mean()
    df["dropoff_rolling_avg"] = df["dropoff_rolling_avg"].fillna(df["dropoff_rate"].mean())

    # --- 1. Per-level drop-off data (with deviation + phase) ---
    dropoff_by_level = []
    for _, row in df.iterrows():
        entry = {
            "level": int(row["level"]),
            "bracket": row["target_bracket"] if pd.notna(row.get("target_bracket")) else None,
            "users": int(row["users"]) if pd.notna(row.get("users")) else None,
            "user_loss": int(row["user_loss"]) if pd.notna(row.get("user_loss")) else 0,
            "dropoff_rate": round(float(row["dropoff_rate"]), 5) if pd.notna(row.get("dropoff_rate")) else 0,
            "rolling_avg": round(float(row["dropoff_rolling_avg"]), 5),
            "expected_dropoff": round(float(row["_expected_dropoff"]), 5) if "_expected_dropoff" in df.columns and pd.notna(row.get("_expected_dropoff")) else None,
            "deviation": round(float(row["_dropoff_deviation"]), 5) if "_dropoff_deviation" in df.columns and pd.notna(row.get("_dropoff_deviation")) else None,
            "deviation_adj": round(float(row["_dropoff_deviation_adj"]), 5) if "_dropoff_deviation_adj" in df.columns and pd.notna(row.get("_dropoff_deviation_adj")) else None,
            "phase": row["_funnel_phase"] if "_funnel_phase" in df.columns else None,
            "aps": round(float(row["aps"]), 3) if "aps" in df.columns and pd.notna(row.get("aps")) else None,
            "combined_churn": round(float(row["combined_churn"]), 4) if "combined_churn" in df.columns and pd.notna(row.get("combined_churn")) else None,
            "completion_rate": round(float(row["completion_rate"]), 4) if "completion_rate" in df.columns and pd.notna(row.get("completion_rate")) else None,
        }
        dropoff_by_level.append(entry)

    # --- 2. Phase-aware spike detection ---
    spikes = _detect_spikes_phase_aware(df)

    # --- 3. Drop-off concentration zones ---
    zones = _find_dropoff_zones(df)

    # --- 4. Phase-level summary ---
    phase_summary = _compute_phase_summary(df)

    # --- 5. Churn correlation ---
    churn_corr = _compute_churn_correlation(df)

    # --- 6. Bracket-level drop-off ---
    bracket_dropoff = _bracket_dropoff_summary(df)

    # --- 7. Insights ---
    insights = _generate_dropoff_insights(df, spikes, zones, churn_corr, bracket_dropoff, phase_summary)

    return {
        "dropoff_by_level": dropoff_by_level,
        "spikes": spikes,
        "zones": zones,
        "phase_summary": phase_summary,
        "churn_correlation": churn_corr,
        "bracket_dropoff": bracket_dropoff,
        "insights": insights,
    }


def _detect_spikes_phase_aware(df):
    """
    Detect levels where drop-off significantly exceeds the expected baseline,
    using phase-adjusted deviation so early-game spikes are de-emphasized
    and late-game spikes are amplified.
    """
    spikes = []

    has_deviation = "_dropoff_deviation_adj" in df.columns
    if not has_deviation:
        # Fall back to old ratio-based detection
        return _detect_spikes_legacy(df)

    # Compute per-phase standard deviation of adjusted deviation
    phase_std = {}
    if "_funnel_phase" in df.columns:
        for phase_name in df["_funnel_phase"].unique():
            phase_df = df[df["_funnel_phase"] == phase_name]
            dev_vals = phase_df["_dropoff_deviation_adj"].dropna()
            if len(dev_vals) >= 3:
                phase_std[phase_name] = max(float(dev_vals.std()), 0.001)
            else:
                phase_std[phase_name] = 0.01

    # Also compute global std as fallback
    global_std = max(float(df["_dropoff_deviation_adj"].dropna().std()), 0.001) if df["_dropoff_deviation_adj"].notna().any() else 0.01

    for _, row in df.iterrows():
        dr = row["dropoff_rate"]
        if pd.isna(dr) or dr < 0.003:
            continue

        dev_adj = row.get("_dropoff_deviation_adj", 0)
        if pd.isna(dev_adj) or dev_adj <= 0:
            continue  # Only flag levels worse than expected

        phase = row.get("_funnel_phase", "Mid")
        std = phase_std.get(phase, global_std)
        sigma = dev_adj / std if std > 0 else 0

        if sigma >= SPIKE_WARNING_SIGMA:
            severity = "critical" if sigma >= SPIKE_CRITICAL_SIGMA else "warning"
            expected = row.get("_expected_dropoff", 0)
            spikes.append({
                "level": int(row["level"]),
                "bracket": row["target_bracket"] if pd.notna(row.get("target_bracket")) else None,
                "dropoff_rate": round(float(dr), 5),
                "expected_dropoff": round(float(expected), 5) if pd.notna(expected) else None,
                "deviation": round(float(row.get("_dropoff_deviation", 0)), 5),
                "deviation_adj": round(float(dev_adj), 5),
                "sigma": round(float(sigma), 2),
                "phase": phase,
                "user_loss": int(row["user_loss"]) if pd.notna(row.get("user_loss")) else 0,
                "severity": severity,
                "aps": round(float(row["aps"]), 3) if "aps" in df.columns and pd.notna(row.get("aps")) else None,
                "combined_churn": round(float(row["combined_churn"]), 4) if "combined_churn" in df.columns and pd.notna(row.get("combined_churn")) else None,
            })

    spikes.sort(key=lambda x: (0 if x["severity"] == "critical" else 1, -x["sigma"]))
    return spikes


def _detect_spikes_legacy(df):
    """Legacy spike detection (ratio-based) as fallback."""
    spikes = []
    for _, row in df.iterrows():
        dr = row["dropoff_rate"]
        avg = row["dropoff_rolling_avg"]
        if dr < 0.003 or avg < 0.001:
            continue
        ratio = dr / avg
        if ratio >= 2.0:
            severity = "critical" if ratio >= 3.5 else "warning"
            spikes.append({
                "level": int(row["level"]),
                "bracket": row["target_bracket"] if pd.notna(row.get("target_bracket")) else None,
                "dropoff_rate": round(float(dr), 5),
                "expected_dropoff": round(float(avg), 5),
                "deviation": round(float(dr - avg), 5),
                "deviation_adj": round(float(dr - avg), 5),
                "sigma": round(float(ratio), 2),
                "phase": row.get("_funnel_phase", "Mid") if "_funnel_phase" in df.columns else None,
                "user_loss": int(row["user_loss"]) if pd.notna(row.get("user_loss")) else 0,
                "severity": severity,
                "aps": round(float(row["aps"]), 3) if "aps" in df.columns and pd.notna(row.get("aps")) else None,
                "combined_churn": round(float(row["combined_churn"]), 4) if "combined_churn" in df.columns and pd.notna(row.get("combined_churn")) else None,
            })
    spikes.sort(key=lambda x: (0 if x["severity"] == "critical" else 1, -x["sigma"]))
    return spikes


def _compute_phase_summary(df):
    """Compute aggregated drop-off stats per funnel phase."""
    summaries = []
    if "_funnel_phase" not in df.columns:
        return summaries

    phase_order = [p["name"] for p in FUNNEL_PHASES]
    for phase_name in phase_order:
        phase_df = df[df["_funnel_phase"] == phase_name]
        if len(phase_df) == 0:
            continue

        level_min = int(phase_df["level"].min())
        level_max = int(phase_df["level"].max())

        entry = {
            "phase": phase_name,
            "level_range": f"{level_min}–{level_max}",
            "level_count": len(phase_df),
            "avg_dropoff": round(float(phase_df["dropoff_rate"].mean()), 5),
            "median_dropoff": round(float(phase_df["dropoff_rate"].median()), 5),
            "max_dropoff": round(float(phase_df["dropoff_rate"].max()), 5),
        }

        if "users" in phase_df.columns and len(phase_df) > 1:
            start_users = int(phase_df["users"].iloc[0])
            end_users = int(phase_df["users"].iloc[-1])
            if start_users > 0:
                entry["retention_through_phase"] = round((end_users / start_users) * 100, 1)
                entry["user_loss"] = start_users - end_users

        if "_dropoff_deviation_adj" in phase_df.columns:
            dev = phase_df["_dropoff_deviation_adj"].dropna()
            if len(dev) > 0:
                entry["avg_deviation_adj"] = round(float(dev.mean()), 5)
                # Count how many levels in this phase are truly problematic
                entry["flagged_levels"] = int((dev > dev.std() * SPIKE_WARNING_SIGMA).sum()) if len(dev) >= 3 else 0

        if "combined_churn" in phase_df.columns:
            churn = phase_df["combined_churn"].dropna()
            if len(churn) > 0:
                entry["avg_churn"] = round(float(churn.mean()), 4)

        if "aps" in phase_df.columns:
            entry["avg_aps"] = round(float(phase_df["aps"].mean()), 3)

        # Dominant bracket in this phase
        if "target_bracket" in phase_df.columns:
            mode_vals = phase_df["target_bracket"].mode()
            entry["dominant_bracket"] = mode_vals.iloc[0] if len(mode_vals) > 0 else None
            entry["bracket_mix"] = {k: int(v) for k, v in phase_df["target_bracket"].value_counts().items()}

        summaries.append(entry)

    return summaries


def _find_dropoff_zones(df):
    """
    Find contiguous zones where cumulative user loss is concentrated.
    Uses a sliding window approach to find the worst zones.
    Now also includes phase context.
    """
    zones = []
    n = len(df)
    if n < 5:
        return zones

    total_users_start = float(df["users"].iloc[0])
    if total_users_start == 0:
        return zones

    window_sizes = [5, 10, 15]
    for w in window_sizes:
        for start in range(0, n - w + 1, max(1, w // 2)):
            end = start + w
            chunk = df.iloc[start:end]
            users_start = float(chunk["users"].iloc[0])
            users_end = float(chunk["users"].iloc[-1])
            if users_start == 0:
                continue

            zone_loss = (users_start - users_end) / total_users_start
            zone_dropoff = (users_start - users_end) / users_start

            if zone_loss >= ZONE_LOSS_WARNING:
                severity = "critical" if zone_loss >= ZONE_LOSS_CRITICAL else "warning"
                dominant = chunk["target_bracket"].mode().iloc[0] if len(chunk["target_bracket"].mode()) > 0 else None
                phase = chunk["_funnel_phase"].mode().iloc[0] if "_funnel_phase" in chunk.columns and len(chunk["_funnel_phase"].mode()) > 0 else None

                zones.append({
                    "start_level": int(chunk["level"].iloc[0]),
                    "end_level": int(chunk["level"].iloc[-1]),
                    "window": w,
                    "funnel_loss_pct": round(float(zone_loss), 4),
                    "zone_dropoff_pct": round(float(zone_dropoff), 4),
                    "user_loss": int(users_start - users_end),
                    "severity": severity,
                    "dominant_bracket": dominant,
                    "phase": phase,
                    "avg_aps": round(float(chunk["aps"].mean()), 3) if "aps" in chunk.columns else None,
                })

    # Deduplicate overlapping zones: keep the highest-loss zone for each start level
    zones.sort(key=lambda z: (-z["funnel_loss_pct"], z["start_level"]))
    seen_levels = set()
    deduped = []
    for z in zones:
        key = z["start_level"]
        if key not in seen_levels:
            deduped.append(z)
            seen_levels.add(key)
        if len(deduped) >= 20:
            break

    return deduped


def _compute_churn_correlation(df):
    """Compute correlation between drop-off rate and churn metrics."""
    corr = {}
    churn_cols = [
        ("combined_churn", "Combined Churn"),
        ("churn", "Session Churn"),
        ("churn_3d", "D3 Churn"),
        ("churn_7d", "D7 Churn"),
    ]

    for col, label in churn_cols:
        if col in df.columns and "dropoff_rate" in df.columns:
            valid = df[[col, "dropoff_rate"]].dropna()
            if len(valid) > 5:
                r = float(valid[col].corr(valid["dropoff_rate"]))
                corr[label] = round(r, 3) if not np.isnan(r) else None

    # Also correlate with APS and completion
    if "aps" in df.columns and "dropoff_rate" in df.columns:
        valid = df[["aps", "dropoff_rate"]].dropna()
        if len(valid) > 5:
            r = float(valid["aps"].corr(valid["dropoff_rate"]))
            corr["APS (Difficulty)"] = round(r, 3) if not np.isnan(r) else None

    if "completion_rate" in df.columns and "dropoff_rate" in df.columns:
        valid = df[["completion_rate", "dropoff_rate"]].dropna()
        if len(valid) > 5:
            r = float(valid["completion_rate"].corr(valid["dropoff_rate"]))
            corr["Completion Rate"] = round(r, 3) if not np.isnan(r) else None

    return corr


def _bracket_dropoff_summary(df):
    """Average drop-off metrics by bracket."""
    result = []
    for bracket in DIFFICULTY_ORDER:
        subset = df[df["target_bracket"] == bracket]
        if len(subset) == 0:
            continue
        result.append({
            "bracket": bracket,
            "count": int(len(subset)),
            "avg_dropoff": round(float(subset["dropoff_rate"].mean()), 5),
            "max_dropoff": round(float(subset["dropoff_rate"].max()), 5),
            "total_user_loss": int(subset["user_loss"].sum()),
            "avg_combined_churn": round(float(subset["combined_churn"].mean()), 4) if "combined_churn" in subset.columns else None,
            "avg_aps": round(float(subset["aps"].mean()), 3) if "aps" in subset.columns else None,
        })
    return result


def _generate_dropoff_insights(df, spikes, zones, churn_corr, bracket_dropoff, phase_summary):
    """Generate human-readable insights about drop-offs, phase-aware."""
    insights = []

    # Total user loss
    if "users" in df.columns and len(df) > 1:
        start_users = int(df["users"].iloc[0])
        end_users = int(df["users"].iloc[-1])
        total_loss = start_users - end_users
        loss_pct = total_loss / start_users * 100 if start_users > 0 else 0
        insights.append(
            f"Total funnel loss: {total_loss:,} users ({loss_pct:.1f}%) across {len(df)} levels."
        )

    # Phase-aware insight: where is the loss concentrated?
    if phase_summary:
        # Find the phase with most user loss (excluding Tutorial which is expected)
        non_tutorial = [p for p in phase_summary if p["phase"] != "Tutorial" and "user_loss" in p]
        if non_tutorial:
            worst_phase = max(non_tutorial, key=lambda p: p.get("user_loss", 0))
            if worst_phase.get("user_loss", 0) > 0:
                insights.append(
                    f"Highest drop-off concentration (excluding onboarding): {worst_phase['phase']} phase "
                    f"(L{worst_phase['level_range']}) — {worst_phase.get('user_loss', 0):,} users lost, "
                    f"avg drop-off {worst_phase['avg_dropoff']*100:.2f}%."
                )

        # Tutorial context
        tutorial = next((p for p in phase_summary if p["phase"] == "Tutorial"), None)
        if tutorial and tutorial.get("retention_through_phase"):
            insights.append(
                f"Tutorial retention: {tutorial['retention_through_phase']:.0f}% of players pass through "
                f"onboarding (L{tutorial['level_range']}). Early drop-off is expected and de-weighted in analysis."
            )

    # Spike summary (now phase-adjusted)
    crit_spikes = [s for s in spikes if s["severity"] == "critical"]
    # Separate early vs mid/late spikes
    mid_late_crits = [s for s in crit_spikes if s.get("phase") in ("Mid", "Late")]
    early_crits = [s for s in crit_spikes if s.get("phase") in ("Tutorial", "Early")]

    if mid_late_crits:
        levels = ", ".join([f"L{s['level']}" for s in mid_late_crits[:5]])
        insights.append(
            f"{len(mid_late_crits)} critical spike(s) in core/late game: {levels}. "
            f"These are high-priority — committed players are leaving."
        )
    if early_crits:
        insights.append(
            f"{len(early_crits)} spike(s) detected in onboarding/early game. "
            f"These are noted but de-prioritized (early churn is partially expected)."
        )

    # Worst zone (skip Tutorial-only zones for priority)
    if zones:
        non_tutorial_zones = [z for z in zones if z.get("phase") != "Tutorial"]
        worst = non_tutorial_zones[0] if non_tutorial_zones else zones[0]
        phase_tag = f" [{worst.get('phase', '')}]" if worst.get("phase") else ""
        insights.append(
            f"Worst drop-off zone{phase_tag}: L{worst['start_level']}–L{worst['end_level']} "
            f"({worst['funnel_loss_pct']*100:.1f}% of total funnel lost, dominant: {worst['dominant_bracket']})."
        )

    # Churn correlation insights
    if "Combined Churn" in churn_corr and churn_corr["Combined Churn"] is not None:
        r = churn_corr["Combined Churn"]
        if r > 0.5:
            insights.append(f"Strong positive correlation (r={r}) between combined churn and drop-off — high-churn levels are where players leave.")
        elif r > 0.3:
            insights.append(f"Moderate correlation (r={r}) between combined churn and drop-off rate.")

    if "APS (Difficulty)" in churn_corr and churn_corr["APS (Difficulty)"] is not None:
        r = churn_corr["APS (Difficulty)"]
        if r > 0.4:
            insights.append(f"Positive correlation (r={r}) between APS and drop-off — harder levels drive more player loss.")
        elif r < -0.2:
            insights.append(f"Negative correlation (r={r}) between APS and drop-off — easier levels may bore players, causing exits.")

    # Bracket with highest drop-off
    if bracket_dropoff:
        worst_bracket = max(bracket_dropoff, key=lambda b: b["avg_dropoff"])
        insights.append(
            f"Highest avg drop-off bracket: {worst_bracket['bracket']} "
            f"({worst_bracket['avg_dropoff']*100:.2f}% per level, {worst_bracket['total_user_loss']:,} users lost total)."
        )

    return insights
